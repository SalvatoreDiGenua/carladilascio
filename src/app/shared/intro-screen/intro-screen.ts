...
@Component({
  ...
  styles: `
    ...
    .intro-screen__veil {
      background:
        radial-gradient(
          ...
          color-mix(
            in srgb,
            var(--primary, var(--color-primary)) 6%,
            transparent
          ),
          transparent 40%
        ),
        linear-gradient(
          ...
          color-mix(
            in srgb,
            var(--background, var(--color-cream)) 97%,
            transparent
          ),
    ...
    .intro-enter {
      animation: intro-enter 900ms cubic-bezier(0.22, 1, 0.36, 1) both;
    }
    .intro-enter .intro-screen__mark {
      animation: intro-mark-enter 1100ms cubic-bezier(0.22, 1, 0.36, 1) 80ms
        both;
    }
    .intro-leave {
      animation: intro-fade-out 720ms cubic-bezier(0.4, 0, 0.2, 1) both;
    }
    .intro-leave .intro-screen__mark {
      animation: intro-mark-out 620ms cubic-bezier(0.4, 0, 0.2, 1) both;
    }
    ...
    @keyframes intro-mark-out {
      ...
      to {
        opacity: 0;
        transform: translate3d(0, -0.25rem, 0) scale(0.99);
        filter: blur(1px);
      }
    }
...
export class IntroScreen {
  ...
  onLeave(event: AnimationCallbackEvent): void {
    ...
    const onAnimationCancel = (e: AnimationEvent) => {
      if (e.target === element) finish();
    };
